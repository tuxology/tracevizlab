...
Click on the ``Show View Filters`` icon at the left of the toolbar and click on the ``Uncheck inactive`` button. This may take a long time (especially if you created the trace on a system with lots of processes running), then the inactive threads will be unchecked from the thread tree. You can then continue checking/unchecking threads as needed. The filter and the result are shown in the following screenshot:
